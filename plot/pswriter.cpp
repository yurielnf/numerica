...
PSWriter::PSWriter(string filename)
    :f(filename.c_str()),tjCmd(""),name(filename)
{
    if (!f.is_open())
...
void PSWriter::Close()
{
    ...
    if (psviewer.is_open())
        cmd=path+" "+name;
    if (system(cmd.c_str()))
        throw runtime_error("Couldn't run a command");
}
...
void PSWriter::SetColor(double red, double green, double blue)
{
    f<<red<<" "<<green<<" "<<blue<<" setrgbcolor"<<endl;
}
